...
@CrossOrigin(origins = "*")
@RequestMapping("/v1")
@Tag(name = "context-export-import-controller", description = "Context Export Import Controller")
public class ContextExportImportController {
    private final ContextExportImportService contextExportImportService;

    @Autowired
    public ContextExportImportController(ContextExportImportService contextExportImportService) {
        this.contextExportImportService = contextExportImportService;
    }

    @RequestMapping(method = {RequestMethod.GET, RequestMethod.POST}, value = "/export/context",
    ...
    public ResponseEntity<Object> exportSystems(@RequestParam(required = false) @Parameter(description = "List of context ids, separated by comma") List<String> systemIds,
        ...
        byte[] zip = contextExportImportService.exportSystemsRequest(systemIds, usedSystemModelIds);
        if (zip == null) {
    ...
    @PostMapping(value = "/import/context")
    public ResponseEntity<List<ImportSystemResult>> importSystems(@RequestParam("file") @Parameter(description = "File") MultipartFile file,
        ...
        List<ImportSystemResult> result = contextExportImportService.importContextSystemRequest(file, systemIds, deployLabel, new HashSet<>(technicalLabels));
        if (result.isEmpty()) {
            return ResponseEntity.noContent().build();
        } else {
    ...
    @PostMapping(value = "/import/contextPreview")
